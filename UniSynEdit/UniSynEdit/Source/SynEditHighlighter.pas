...
type
  TCodeFoldingSkipFunc = function(var Ptr: PChar; var Line: Integer): Boolean of
    object; // pjura
  ...
  public
    ...
    procedure SetLineExpandedAtWideGlyphs(const Line, ExpandedLine:
      UnicodeString;
    ...
    procedure CodeFoldingLoadFromFile(FileName: string);
    procedure CodeFoldingLoadFromStream(aStreem: TStream);
...
{$ENDIF}
    ...
    property FriendlyLanguageName: UnicodeString read
      GetFriendlyLanguageNameProp;
  ...
  public
    ...
    property SampleSource: UnicodeString read GetSampleSource write
      SetSampleSource;
...
function TSynHighlighterList.GetItem(Index: Integer):
  TSynCustomHighlighterClass;
begin
...
procedure TSynHighlighterAttributes.AssignColorAndStyle(Source:
  TSynHighlighterAttributes);
var
  bChanged: Boolean;
begin
...
constructor TSynHighlighterAttributes.Create(AName: string; AFriendlyName:
  UnicodeString);
begin
  inherited Create;
  Background := clNone;
          ...
          begin
            try
              if ValueExists('Foreground Color') then
                fgColor := Pal16[ReadInteger('Foreground Color')]
              else if ValueExists('Foreground Color New') then
                fgColor := StringToColor(ReadString('Foreground Color New'))
              else
                Exit;
              if ValueExists('Background Color') then
                bgColor := Pal16[ReadInteger('Background Color')]
              ...
              if ValueExists('Bold') then
                fontBold := ReadString('Bold')
              else
                Exit;
              if ValueExists('Italic') then
                fontItalic := ReadString('Italic')
              else
                Exit;
              if ValueExists('Underline') then
                fontUnderline := ReadString('Underline')
              else
                Exit;
              if ValueExists('Default Foreground') then
                fgDefault := ReadString('Default Foreground')
              else
                Exit;
              if ValueExists('Default Background') then
                bgDefault := ReadString('Default Background')
              else
                Exit;
              if IsTrue(bgDefault) then
                Background := clWindow
              else
                Background := bgColor;
              if IsTrue(fgDefault) then
                Foreground := clWindowText
              else
                Foreground := fgColor;
              Style := [];
              if IsTrue(fontBold) then
                Style := Style + [fsBold];
              if IsTrue(fontItalic) then
                Style := Style + [fsItalic];
              if IsTrue(fontUnderline) then
                Style := Style + [fsUnderline];
              Result := True;
            finally
              CloseKey;
...
function TSynHighlighterAttributes.LoadFromRegistry(Reg: TBetterRegistry):
  Boolean;
var
  Key: string;
begin
...
function TSynHighlighterAttributes.SaveToRegistry(Reg: TBetterRegistry):
  Boolean;
var
  Key: string;
begin
...
begin
  ...
  TypeFile := GetLanguageName + '.xml';
  AppPath := IncludeTrailingBackslash(ExtractFilePath(Application.ExeName));

  if FileExists(AppPath + 'DocumentTypes\' + TypeFile) then
  begin
    CodeFoldingLoadFromFile(AppPath + 'DocumentTypes\' +
      TypeFile);
  end
  else
  begin
    if not (FindResource(Hinstance, PChar(TypeFile), RT_RCDATA) = 0) then
    begin
      rs := TResourceStream.Create(hinstance, TypeFile, RT_RCDATA);
      try
        rs.Position := 0;
        CodeFoldingLoadFromStream(rs);
      finally
        rs.Free;
      end;
    end;
...
class function TSynCustomHighlighter.GetCapabilities:
...
{$IFDEF SYN_DEVELOPMENT_CHECKS}
  raise Exception.CreateFmt('%s.GetFriendlyLanguageName not implemented',
    [ClassName]);
{$ENDIF}
...
function TSynCustomHighlighter.IsCurrentToken(const Token: UnicodeString):
  Boolean;
var
  I: Integer;
  Temp: PWideChar;
...
function TSynCustomHighlighter.IsKeyword(const AKeyword: UnicodeString):
  Boolean;
begin
  Result := False;
end;
...
procedure TSynCustomHighlighter.SetLine(const Value: UnicodeString; LineNumber:
  Integer);
begin
  fExpandedLineStr := '';
  fExpandedLine := nil;
...
procedure TSynCustomHighlighter.UnhookAttrChangeEvent(ANotifyEvent:
  TNotifyEvent);
begin
...
procedure TSynCustomHighlighter.ReadCodeFolding(xml: TXMLParser;
...
begin
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          else if (xml.CurPartType = ptEndTag) and (Verify('SkipRegions', xml))
            then
            Break;
        end
      ...
      begin
        if ParentFold = nil then
          FoldRegion := FoldRegions.Add(FoldRegionType, AddCloseKeyWord,
            AllowNoSubFolds, WholeWords, PChar(OpenKeyWord),
            PChar(CloseKeyWord),
            ParentFold)
        else
    ...
    begin
      ...
      and ((xml.CurName = 'SubFoldRegions') or (xml.CurName = 'CodeFolding'))
        then
      Exit;
  end;
end;
...
procedure TSynCustomHighlighter.LoadFromXml(xml: TXMLParser);
begin
  while xml.Scan do
  begin
    if (xml.CurPartType = ptStartTag) then
...
procedure TSynCustomHighlighter.CodeFoldingLoadFromFile(FileName: string);
var
  xml: TXMLParser;
begin
  if not FileExists(FileName) then
    raise Exception.Create(ClassName + '.LoadFromFile - "' + FileName +
      '" does not exists.');
  xml := TXMLParser.Create;
  try
    if xml.LoadFromFile(FileName) then
    begin
      xml.StartScan;
      LoadFromXml(xml);
    end;
...
procedure TSynCustomHighlighter.CodeFoldingLoadFromStream(aStreem: TStream);
var
  xml: TXMLParser;
begin
  if aStreem = nil then
    raise Exception.Create(ClassName +
      '.LoadFromStream - Stream does not exists.');
  xml := TXMLParser.Create;
  try
    TCustomMemoryStream(aStreem).Position := 0;
    if xml.LoadFromBuffer(TCustomMemoryStream(aStreem).Memory) then
    begin
      xml.StartScan;
      LoadFromXml(xml);
    end;
