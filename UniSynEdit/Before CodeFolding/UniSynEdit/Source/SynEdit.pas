...
{$ENDIF}
  ...
  protected
    ...
    procedure DoContextPopup(MousePos: TPoint; var Handled: Boolean); override; // KBFix
...
{$ENDIF}
    ...
    function IsWordBreakNoSpaceCharNeg(AChar: WideChar): Boolean; virtual; // KBFix
...
// KBFix
  if (X < fGutterWidth) and ([ssShift, ssAlt, ssCtrl] * Shift = []) then  // Krystian: 11-08-2007
  begin
    if Button = mbRight then
...
// KBFix BEGIN
      if IsIdentChar(Line[CX]) then
        ...
        if IsWordBreakNoSpaceChar(Line[CX]) then
...
// KBFix BEGIN
      ...
      if (CX > 1) and (CX - 1 <= iLineLen) then
        if IsWordBreakNoSpaceChar(Line[CX - 1]) then
          CX := StrRScanForCharInCategory(Line, CX - 1, IsWordBreakNoSpaceCharNeg) + 1
        else
