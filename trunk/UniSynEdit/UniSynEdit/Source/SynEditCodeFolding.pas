...
The Original Code is: SynEditCodeFolding.pas, released 2005.
The Original Code is based on Mystix.pas by Piotr Jura, part of
the Mystix Text Editor.
Portions created by Piotr Jura are Copyright (C) 2005 Piotr Jura.
Unicode translation by Voropay Andrey 2010.
All Rights Reserved.
...
uses
  Graphics, Types, Classes, SysUtils, SynUnicode;
...
type
  ...
  public
    ...
    property BeforeFirstLineState: Integer read fBeforeFirstLineState write fBeforeFirstLineState;
...
begin
  ...
  begin
    ...
    Result := (RoundUp(B * 255) shl 16) + (RoundUp(G * 255) shl 8) + RoundUp(R *
      255);
  end;
end;
...
var
  ...
  begin
    ...
    begin
      ...
      begin
        FoldRange := FoldRanges.Add(Self, fFromLine, fLevel, fRealLevel,
          fFoldRegion, fToLine);
...
function TSynEditFoldRange.CollapsableFoldRangeForLine(Line: Integer;
  FoldCount: PInteger): TSynEditFoldRange;
var
  SubLine,i, LastLine: Integer;
  FoldRange : TSynEditFoldRange;
begin
  Result := nil;
  LastLine := 0;
...
//    with SubFoldRanges[i] do begin
     FoldRange := fSubFoldRanges[i];
     SubLine := FoldRange.FromLine;
      if {(not fSubFoldRanges[i].ParentCollapsed) and} (SubLine
        = Line) {and (fSubFoldRanges[i].Collapsable) }then
      begin
        Result := fSubFoldRanges[i];
        Break;
      end
...
function TSkipRegions.Add(const aOpen, aClose, aEscape: string;
  aType: TSkipRegionItemType): TSkipRegionItem;
begin
  with TSkipRegionItem(inherited Add) do
  begin
    Open := PChar(aOpen);
    Close := PChar(aClose);
