...
begin
  ...
  begin
    ...
    {$ENDIF}
      ; //Params.WindowClass.style := Params.WindowClass.style or CS_DROPSHADOW; // KBFix: no shadow=no flicker
...
procedure TSynBaseCompletionProposal.ExecuteEx(s: UnicodeString; x, y: integer; Kind : SynCompletionType);
  ...
  begin
    ...
    if (tmpY + tmpHeight > GetWorkAreaHeight) and not (Form.CurrentEditor = nil) then
    begin
