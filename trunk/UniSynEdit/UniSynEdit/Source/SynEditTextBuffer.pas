...
type
  ...
  TSynEditStringState = (ssfUnchanged, ssfChanged, ssfSaved);
...
type
  ...
  TStringListDefinePropertiesEvent = procedure of object;
  ...
  private
    ...
    fOnDefineProperties: TStringListDefinePropertiesEvent;
    ...
    function GetStringStates(Index: integer): TSynEditStringState;
    procedure Grow;
    procedure InsertItem(Index: integer; const S: UnicodeString);
    procedure PutRange(Index: integer; ARange: TSynEditRange);
    procedure PutStringStates(Index: integer; AStringState: TSynEditStringState);
...
{$ENDIF}
    ...
    property OnInserted: TStringListChangeEvent read fOnInserted
      write fOnInserted;
    property OnDefineProperties: TStringListDefinePropertiesEvent read fOnDefineProperties
      write fOnDefineProperties;
    property OnPutted: TStringListChangeEvent read fOnPutted write fOnPutted;
    property StringStates[Index: integer]: TSynEditStringState read GetStringStates write PutStringStates;
  end;
  ...
  public
    ...
    property StringStates: TStrings read fStringStates;
  end;
  ...
  public
    constructor Create(aLines:TSynEditStringList);
    destructor Destroy; override;
  ...
  private
    ...
    fStringStates:TStringList;
    procedure FillStringStatesList;
  public
    procedure Refresh;
  end;
...
procedure TSynEditStringList.DefineProperties(Filer: TFiler);
begin
  inherited;
...
function TSynEditStringList.GetStringStates(Index: integer): TSynEditStringState;
begin
  if (Index >= 0) and (Index < fCount) then
    Result := fList^[Index].fState
  else
    Result := ssfUnchanged;
end;
...
procedure TSynEditStringList.PutStringStates(Index: integer; AStringState: TSynEditStringState);
begin
  if (Index < 0) or (Index >= fCount) then
    ListIndexOutOfBounds(Index);
//  BeginUpdate;
  fList^[Index].fState := AStringState;
//  EndUpdate;
end;
...
// TODO: IFDEF UNICODE, LineBreak setzen
  if fLINESEPARATOR then
    ...
    fFileFormat := sffDos;
  //Clear line changes flags when load new text to TSynEditStringList
  for ipos:=0 to fCount-1 do
    StringStates[ipos]:=ssfUnchanged;
end;
...
begin
  ...
  begin
    ...
    fStringStates:=TSynEditUndoItem(Source).fStringStates;
  end
  else
...
constructor TSynEditUndoList.Create(aLines:TSynEditStringList);
begin
  inherited Create;
  fLines:=aLines;
  fItems := TList.Create;
  ...
  fNextChangeNumber := 1;
  fInsideRedo := False;
  fStringStates:=TStringList.Create;
  FillStringStatesList;
end;
...
procedure TSynEditUndoList.FillStringStatesList;
  var i:Integer;
  begin
    if not Assigned(fStringStates) then
      begin
      fStringStates:= TStringList.Create;
      for i:=0 to fLines.Count-1 do
        begin
        fStringStates.Add(IntToStr(Integer(TSynEditStringList(fLines).StringStates[i])))
        end;
      end;
  end;

procedure TSynEditUndoList.Refresh;
begin
  if assigned(fStringStates) then
    begin
    fStringStates.free;
    fStringStates:=nil;
    end;
...
begin
  ...
  begin
    ...
    fLines:=TSynEditUndoList(Source).fLines;
    fStringStates:=TStringList.Create;
    FillStringStatesList;
  end
  else
  ...
  if fLockCount = 0 then begin
    ...
    try
      with NewItem do begin
        ...
        fStringStates:=Self.fStringStates;
        Self.fStringStates:=nil;
        end;
      PushItem(NewItem);
    except
      NewItem.Free;
...
constructor TSynEditUndoItem.Create;
  begin
  fStringStates:=nil;
  end;

destructor TSynEditUndoItem.Destroy;
  begin
  if assigned(fStringStates) then
    fStringStates.Free;
  end;

procedure TSynEditStringList.Assign(Source: TPersistent);
var i:Integer;
begin
  inherited;
  ...
  begin
    for i:=0 to count-1 do
      StringStates[i]:=TSynEditStringList(Source).StringStates[i];
  end;
end;
