...
13.05.2010 Modifiend: Voropay Andrey
This program was rewritten for the aims of the use with
the set of components of package of UniSynEdit v.1.386.2.75 2008/09/17

To contact with me you can on e-mail: avoropay@ukr.net

16.11.2010 Modified: Mihail Slobodyanuk (slobodyanukma@ukr.net, slobodyanukma@gmail.com)
Code folding now work in visualisation phase. Lines of SinEdit not modifiable of Code Folding
-------------------------------------------------------------------------------}
...
{$ENDIF}
  ...
  TDropFilesEvent = procedure(Sender: TObject; X, Y: Integer; AFiles:
    TUnicodeStrings)
    of object;
  ...
  TProcessCommandEvent = procedure(Sender: TObject;
    var Command: TSynEditorCommand; var AChar: WideChar; Data: pointer) of
    object;

  TReplaceTextEvent = procedure(Sender: TObject; const ASearch, AReplace:
    UnicodeString; Line, Column: Integer; var Action: TSynReplaceAction) of
    object;
  ...
  TScrollEvent = procedure(Sender: TObject; ScrollBar: TScrollBarKind) of
    object;
...
type
  ...
  TMouseCursorEvent = procedure(Sender: TObject; const aLineCharPos:
    TBufferCoord;
  ...
  public
    property Items[Index: Integer]: TSynEditMark read GetItem write SetItem;
    default;
    property Edit: TCustomSynEdit read fEdit;
...
{$ENDIF}
  ...
  TIndexArray = array of TLineIndex;
...
{$IFNDEF SYN_CLX}
    ...
    procedure WMGetTextLength(var Msg: TWMGetTextLength); message
      WM_GETTEXTLENGTH;
...
{$ENDIF}
  private
    //### Code Folding ###
    fNeedToReScan: Boolean;
    fAllFoldRanges: TSynEditAllFoldRanges;
    fHighlightedFoldRange: TSynEditFoldRange;
    fHint: TObject;
    fIndGuidBmp: Graphics.TBitmap;
    fIndGuidOddBmp: Graphics.TBitmap;
    fHIndGuidBmp: Graphics.TBitmap;
    fHIndGuidOddBmp: Graphics.TBitmap;
    fCodeFolding: TSynCodeFolding;
    fIsMouseOnHintRect: Boolean;
    fRowByLine: TIndexArray;
    fLineByRow: TIndexArray;
    fLineByRowLength: Integer;
    fRowCount: Integer;
    fNeedRescanLineAndRow: Boolean;
    fInvalidateLineEnabled: Boolean;
    //### End Code Folding ###
...
{$IFDEF SYN_CLX}
    ...
    procedure ScrollEvent(Sender: TObject; ScrollCode: TScrollCode; var
      ScrollPos: Integer);
{$ENDIF}

    //### Code Folding ###
    {Return range, which begin from Line}
    function FoldRangeForLine(Line: Integer): TSynEditFoldRange;
    {Return collapsable range, which begin from Line}
    function CollapsableFoldRangeForLine(Line: Integer;
      FoldCount: PInteger = nil): TSynEditFoldRange;
    {Return top collapsed range, which contain Line (if exist) or nil}
    function CollapsedFoldRangeByLine(Line: Integer): TSynEditFoldRange;
    {Return true if at least one Range contain line}
    function IsLineInFoldRange(Line: Integer): Boolean;
    {Return range, which end at Line}
    function FoldRangeForLineTo(Line: Integer): TSynEditFoldRange;
    function GetLineIndent(Strings: TUnicodeStrings; ALine: Integer): Integer;
    function TreeEndForRow(Row: Integer): Boolean;
    function TreeLineForRow(Row: Integer): Boolean;
    function LastRowForLine(Line: Integer): Integer;
    {Collapse range}
    procedure Collapse(AFoldRange: TSynEditFoldRange);
    procedure ScanForFoldRanges(var TopFoldRanges: TSynEditAllFoldRanges;
      Strings: TUnicodeStrings);
    {Expand range}
    procedure Expand(FoldRange: TSynEditFoldRange);
    procedure ExpandLine(Line: Integer);
    ...
    procedure RescanLineAndRow(index: Integer = 1);
    function GetRowLength(aRow: integer): integer;
    //### End Code Folding ###
    ...
    function LeftSpacesEx(const Line: UnicodeString; WantTabs: Boolean):
      Integer;
    function GetLeftSpacing(CharCount: Integer; WantTabs: Boolean):
      UnicodeString;
...
{$ENDIF}
    ...
    procedure ListDefineProperties; virtual;
    ...
    procedure ChainListDeleted(Sender: TObject; aIndex: Integer; aCount:
      Integer);
    procedure ChainListInserted(Sender: TObject; aIndex: Integer; aCount:
      Integer);
    procedure ChainListPutted(Sender: TObject; aIndex: Integer; aCount:
      Integer);
    ...
    procedure SetSelTextPrimitiveEx(PasteMode: TSynSelectionMode; Value:
      PWideChar;
      AddToUndoList: Boolean);
  ...
  protected
    ...
    procedure DoOnPaintTransientEx(TransientType: TTransientType; Lock: Boolean);
      virtual;
...
{$ENDIF}
    ...
    function GetMatchingBracketEx(const APoint: TBufferCoord): TBufferCoord;
      virtual;
{$IFDEF SYN_COMPILER_4_UP}
...
{$ENDIF}
    ...
    function GetHighlighterAttriAtRowCol(const XY: TBufferCoord; var Token:
      UnicodeString;
      var Attri: TSynHighlighterAttributes): Boolean;
    function GetHighlighterAttriAtRowColEx(const XY: TBufferCoord; var Token:
      UnicodeString;
    ...
    function IsIdentCharNeg(AChar: WideChar): Boolean; virtual; // KBFix

    function IsWhiteChar(AChar: WideChar): Boolean; virtual;
    function IsWhiteCharNeg(AChar: WideChar): Boolean; virtual; // KBFix

    function IsWordBreakChar(AChar: WideChar): Boolean; virtual;
    function IsWordBreakCharNeg(AChar: WideChar): Boolean; virtual; // KBFix

    function IsWordBreakNoSpaceChar(AChar: WideChar): Boolean; virtual; // KBFix
    function IsWordBreakNoSpaceCharNeg(AChar: WideChar): Boolean; virtual;
    // KBFix
    ...
    procedure LockUndo;
    function DisplayToBufferLine(const row: Integer): Integer;
    function BufferToDisplayRow(const line: Integer): Integer;
    ...
    procedure SetCaretAndSelection(const ptCaret, ptBefore, ptAfter:
      TBufferCoord);
...
{$ENDIF}
  ...
  public
    ...
    procedure ExpandLevel(ALevel: Integer; NeedInvalidate: Boolean = True);
    procedure ExpandAllParent(vFoldRange: TSynEditFoldRange);
    //### End Code Folding ###
  public
    //### Code Folding ###
    ...
    property SearchEngine: TSynEditSearchCustom read fSearchEngine write
      SetSearchEngine;
  ...
  public
    ...
    property WordWrapGlyph: TSynGlyph read fWordWrapGlyph write
      SetWordWrapGlyph;
    property OnChange: TNotifyEvent read FOnChange write FOnChange;
...
const
  FrameWidth = 2;
  { the border width when BoderStyle = bsSingle (until we support TWidgetStyle...)  }
{$ENDIF}
...
begin
  if SelAvail then
  begin
    ChangeTrim := (fActiveSelectionMode = smColumn) and (eoTrimTrailingSpaces in
      Options);
    try
      if ChangeTrim then
...
begin
  ...
  SetLength(fLineByRow, 2);
  fLineByRowLength:=1;
  SetLength(fRowByLine, 2);
  fNeedRescanLineAndRow:=true;
  fInvalidateLineEnabled:=true;
  //### End Code Folding ###
  ...
  begin
    ...
    OnPutted := ListPutted;
    OnDefineProperties := ListDefineProperties;
  end;
  fFontDummy := TFont.Create;
  fUndoList := TSynEditUndoList.Create(TSynEditStringList(flines));
  fUndoList.OnAddedUndo := UndoRedoAdded;
  fOrigUndoList := fUndoList;
  fRedoList := TSynEditUndoList.Create(TSynEditStringList(flines));
...
begin
  ...
  begin
    ...
    if (WordWrap or CodeFolding.Enabled) and ([scCaretX, scCaretY] * fStatusChanges <> []) then
    begin
      vAuxPos := DisplayXY;
...
begin
  if (fBlockEnd.Line < fBlockBegin.Line)
    or ((fBlockEnd.Line = fBlockBegin.Line) and (fBlockEnd.Char <
    fBlockBegin.Char)) then
    Result := fBlockEnd
  else
    Result := fBlockBegin;
end;
...
begin
  Result := (fBlockBegin.Char <> fBlockEnd.Char) or
    ((fBlockBegin.Line <> fBlockEnd.Line) and (fActiveSelectionMode <>
    smColumn));
end;

function TCustomSynEdit.GetSelTabBlock: Boolean;
begin
  Result := (fBlockBegin.Line <> fBlockEnd.Line) and (fActiveSelectionMode <>
    smColumn);
end;

function TCustomSynEdit.GetSelTabLine: Boolean;
begin
  Result := (BlockBegin.Char <= 1) and (BlockEnd.Char > length(Lines[CaretY -
    1])) and SelAvail;
end;
    ...
    case fActiveSelectionMode of
      ...
      smLine:
        begin
          ...
          if Last = Lines.Count - 1 then // KBFix
...
begin
  Result := GetWordAtRowCol(CaretXY); // KBFix
  //var
  //  bBegin: TBufferCoord;
  //  bEnd: TBufferCoord;
  //begin
...
var
  rcInval: TRect;
begin
  if not fInvalidateLineEnabled then exit;
...
{$ENDIF}
    ...
    begin
      ...
      if WordWrap or CodeFolding.Enabled then
      begin
...
var
  rcInval: TRect;
  CollapsedFoldRange: TSynEditFoldRange;
begin
  if not fInvalidateLineEnabled then Exit;
  if Visible and HandleAllocated then
    if (FirstLine = -1) and (LastLine = -1) then
    ...
    begin
      ...
      begin
        ...
        if LastLine < Lines.Count then
          LastLine := LastRowForLine(LastLine);
      end;
      if CodeFolding.Enabled then
      begin
        CollapsedFoldRange := CollapsableFoldRangeForLine(FirstLine);
        if not((CollapsedFoldRange <> nil) and (CollapsedFoldRange.Collapsed)
            and (not CollapsedFoldRange.ParentCollapsed)) then
          CollapsedFoldRange := nil;
        if (CollapsedFoldRange <> nil) and (FirstLine>LastLine) then
           LastLine:=FirstLine;
      end;
...
function TCustomSynEdit.LeftSpacesEx(const Line: UnicodeString; WantTabs:
  Boolean): Integer;
var
  p: PWideChar;
begin
...
procedure TCustomSynEdit.ListDefineProperties;
begin
  fUndoList.Refresh;
end;
...
{$IFDEF SYN_CLX}
  if not PtInRect(GetClientRect, Point(X, Y)) then
    Exit;
{$ENDIF}
  if fIsMouseOnHintRect then
    Exit;

  if (X < fGutterWidth + 2) then // KBFix
    ComputeCaret(X, Y);
  ...
  if (Button in [mbLeft, mbRight]) and (X >= fGutterWidth + 2) then // KBFix
  begin
  ...
  begin
    ...
    if bWasSel and (eoDragDropEditing in fOptions) and (X >= fGutterWidth + 2)
      and (SelectionMode = smNormal) and
  ...
  begin
    ...
    begin
      ...
      begin
        if (eoAltSetsColumnMode in Options) and (fActiveSelectionMode <> smLine)
          then
        begin
  ...
  // KBFix
  if (X < fGutterWidth) and ([ssShift, ssAlt, ssCtrl] * Shift = []) then
    // Krystian: 11-08-2007
  begin
    if Button = mbRight then
...
{$ENDIF}
  ...
  begin
    fIsMouseOnHintRect:=False;
    P := PixelsToRowColumn(X, Y);
    FoldRange := CollapsableFoldRangeForLine(RowToLine(P.Row));

    if (FoldRange <> nil) and (FoldRange.Collapsed) and
      ((not WordWrap) or (LastRowForLine(RowToLine(P.Row))=P.Row))then
    begin
      ScrolledXBy := (LeftChar - 1) * CharWidth;
      Pt.X := X;
      Pt.Y := Y;
      // http://mystix.projects.waterproof.fr/IssueView.php?Id=43
      Rect := GetCollapseMarkRect(FoldRange, P.Row);

      if Rect.Right - ScrolledXBy > 0 then
      begin
        OffsetRect(Rect, -ScrolledXBy, 0);
        if PtInRect(Rect, Pt) then
        begin
          fIsMouseOnHintRect:=True;
          Cursor := crArrow;

          if TSynCompletionProposal(fHint).Tag = 0 then
          begin
            TSynCompletionProposal(fHint).ItemList.Clear;
            for i := FoldRange.FromLine - 1 to FoldRange.ToLine -1  do
              TSynCompletionProposal(fHint).ItemList.Add(
                StringReplace(lines[i], #9,
                StringOfChar(#32, TabWidth), [rfReplaceAll]));


            TSynCompletionProposal(fHint).Editor := Self;

            TSynCompletionProposal(fHint).ExecuteEx('', X + 10 + ClientOrigin.X,
              Y + ClientOrigin.Y, ctHint);
            //            TSynCompletionProposal(fHint).ExecuteEx('', X + 10, Y + Top,
            //              ctHint);
...
{$ENDIF}
  ...
  begin
    ...
    BlockEnd := CaretXY;
    if (sfPossibleGutterClick in fStateFlags) and (FBlockBegin.Line <>
      CaretXY.Line) then
    begin
      Exclude(fStateFlags, sfPossibleGutterClick); // KBFix
...
begin
  ...
  if (Button = mbRight) and (Shift = [ssRight]) and Assigned(PopupMenu) then
    exit;
  MouseCapture := False;
  if (sfPossibleGutterClick in fStateFlags) and (X < fGutterWidth) and (Button
    <> mbRight) then
  begin
    DoOnGutterClick(Button, X, Y)
  end
...
begin
  ...
  if CodeFolding.Enabled then
  begin
    FoldRange := CollapsableFoldRangeForLine(RowToLine(PixelsToRowColumn(X,
      Y).Row));
  ...
  begin
    line := DisplayToBufferLine(PixelsToRowColumn(X, Y).Row);
    ...
    begin
      ...
      for i := Low(allmrk) to High(allmrk) do // KBFix
      begin
...
{$ENDIF}
  ...
  try
    ...
    //### Code Folding ###
    if (CodeFolding.Enabled) and (Gutter.Visible) then
      for cRow := fTopLine to (fTopLine + fLinesInWindow) do
      begin
        if cRow<=fRowCount then
        begin
          cLine:=RowToLine(cRow);
          //### Change in 0.1.5 ###
           // http://mystix.projects.waterproof.fr/IssueView.php?Id=10
          if Gutter.ShowLineNumbers then
            xpos := Gutter.LeftOffset + Max(GetAutoSizeDigitCount,
              Gutter.DigitCount) * CharWidth + 4
          else
            xpos := Gutter.Width - Gutter.RightOffset + 4;

          Inc(xpos);
          rect.Top := (cRow - fTopLine) * LineHeight;
          ...
          if TreeEndForRow(cRow) then
          begin
            x := rect.Left + ((rect.Right - rect.Left) div 2);
            Canvas.MoveTo(x, rect.Top);
            Canvas.LineTo(x, rect.Top + ((rect.Bottom - rect.Top) div 2));
            Canvas.LineTo(rect.Right - 2, rect.Top + ((rect.Bottom - rect.Top) div
              2));
          end;

          if (cRow = 1) or (cLine <> RowToLine(cRow - 1)) then
          begin
            //### Change in 0.1.3 ###
            // http://mystix.projects.waterproof.fr/IssueView.php?Id=8
            FoldRange := CollapsableFoldRangeForLine(cLine);
            ...
            begin
              ...
              // Paint minus sign
              Canvas.Pen.Color := CodeFolding.FolderBarLinesColor;
              Canvas.MoveTo(rect.Left + 2, rect.Top + ((rect.Bottom - rect.Top)
                div
                2));
              Canvas.LineTo(rect.Right - 2, rect.Top + ((rect.Bottom - rect.Top)
                div 2));
...
var
  ...
  FoldRange: TSynEditFoldRange;
  //### End Code Folding ###
  StringState: TSynEditStringState;
{$IFNDEF SYN_CLX}
  dc: HDC;
  TextSize: TSize;
{$ENDIF}
begin
  ...
  begin
    if Gutter.ShowLineNumbers then
      xpos := Gutter.LeftOffset + Max(GetAutoSizeDigitCount, Gutter.DigitCount)
        * CharWidth + 4
    else
      xpos := Gutter.Width - Gutter.RightOffset + 4;
  end
...
{$ENDIF}
  ...
  if fGutter.Gradient then
    SynDrawGradient(Canvas, fGutter.GradientStartColor,
      fGutter.GradientEndColor,
      fGutter.GradientSteps, Classes.Rect(0, 0, fGutterWidth, ClientHeight), True);
...
{$ENDIF}
    try
      ...
      //### Code Folding ###
      if CodeFolding.Enabled then
        rcLine.Right := Gutter.LeftOffset
          + Max(GetAutoSizeDigitCount, Gutter.DigitCount) * CharWidth + 4
      else
      ...
      begin
        FoldRange:=nil;
        if CodeFolding.Enabled then
          FoldRange := CollapsedFoldRangeByLine(cLine);
        if (FoldRange = nil) or (FoldRange.FromLine=cLine) then
        begin
  ...
  {$ELSE}
          ...
          begin
            ...
            Windows.ExtTextOutW(DC, (fGutterWidth - fGutter.RightOffset - 2) -
              TextSize.cx,
            ...
            Windows.ExtTextOutW(DC, (fGutterWidth - fGutter.RightOffset - 2) -
              TextSize.cx,
              rcLine.Top + ((fTextHeight - Integer(TextSize.cy)) div 2),
              ETO_OPAQUE,
              @rcLine, PWideChar(s), Length(s), nil);
{$ENDIF}
        end;
  ...
  //Draw StringState mark
  if fGutter.HighlightChanges then
    ...
    begin
      ...
      rcLine.Top := vLineTop;
      rcLine.Bottom := rcLine.Top + fTextHeight;
        StringState := TSynEditStringList(fLines).StringStates[RowToLine(cRow) - 1];
        if (StringState=ssfChanged) or (StringState=ssfSaved) then
          begin
          if StringState=ssfChanged then
            Canvas.Brush.Color := clYellow
          else if StringState=ssfSaved then
            Canvas.Brush.Color := clGreen;
          rect := rcLine;
          if CodeFolding.Enabled then
            rect.Left := xpos+1
          else
            rect.Left := fGutterWidth-7;
          rect.Right := rect.Left+5;
          Canvas.FillRect(rect);
          end;
          Canvas.Brush.Color := fGutter.Color;
    end;
  //End Draw StringState mark
  ...
  begin
    ...
    try
      ...
      for cMark := 0 to Marks.Count - 1 do
        with Marks[cMark] do
        begin
          if Visible and (Line >= vFirstLine)
              and (Line <= vLastLine) then
          begin
...
function TCustomSynEdit.ShrinkAtWideGlyphs(const S: UnicodeString; First:
  Integer;
  var CharCount: Integer): UnicodeString;
var
  i, j: Integer;
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        bAnySelection := (vEnd.Line >= vFirstLine) and (vStart.Line <=
          vLastLine);
        if bAnySelection then
        begin
          ...
          if (fActiveSelectionMode = smColumn) and (vSelStart.Column >
            vSelEnd.Column) then
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        CountOfAvgGlyphs := Ceil(fTextDrawer.TextWidth(Token[NonFillerPos]) /
          fCharWidth);
        ...
        if WordWrap then
          ...
          begin
            dec(nCharsToPaint);
            while (nCharsToPaint > 0) and (Token[First + nCharsToPaint - 1] =
              FillerChar) do
              dec(nCharsToPaint);
          end;
...
{$ENDIF}
  ...
  var
    ...
    vEndRow: Integer;
    //### Code Folding ###
    i, FoldCount, CurrentFold, X, Y, ScrolledXBy, j: Integer;
    FoldRange, CollapsedFoldRange: TSynEditFoldRange;
    RowCol: TBufferCoord;
    rcCollapseMark: TRect;
    IndentGuides: array of array of Integer;
    //### End Code Folding ###
  begin
    ...
    ScrolledXBy := (LeftChar - 1) * CharWidth;
    SetLength(IndentGuides, Lines.Count);

    with fAllFoldRanges do
      for i := 0 to AllCount - 1 do
        with AllFoldRanges[i] do
          if (not Collapsed) and (not ParentCollapsed) then
          begin
            X := Level * CharWidth * TabWidth;

            if X - ScrolledXBy > 0 then
            begin
              X := Gutter.RealGutterWidth(CharWidth) + X - ScrolledXBy;

              if (FromLine >= 0) and (ToLine >= 0) then
                for j := FromLine to ToLine do
                begin
                  SetLength(IndentGuides[Pred(j)],
                    Succ(Length(IndentGuides[Pred(j)])));
                  IndentGuides[Pred(j)][High(IndentGuides[Pred(j)])] := X;
                end;
            end;
          end;
    //### End Code Folding ###
    ...
    begin
      ...
      if (FoldRange <> nil) and (FoldRange.Collapsed) and (not FoldRange.ParentCollapsed) then
        CollapsedFoldRange:=FoldRange;
      //clear pointer to old range
      if Assigned(CollapsedFoldRange) and (CollapsedFoldRange.ToLine<nLine) then
        CollapsedFoldRange:=nil;
      //### End Code Folding ###
      sLine := TSynEditStringList(Lines).ExpandedStrings[nLine - 1];
      if (CollapsedFoldRange <> nil) and (CollapsedFoldRange.FromLine=nLine)
          and CollapsedFoldRange.FoldRegion.AddEnding then
        sLine:=sLine+flines[CollapsedFoldRange.ToLine-1];
      ...
      if vStartRow=0 then
        ...
        vEndRow := Min(LastRowForLine(nLine), aLastRow);
      for cRow := vStartRow to vEndRow do
      begin
        vFirstChar := FirstCol;
        vLastChar := LastCol;
        //if WordWrap and (CollapsedFoldRange = nil) then
        if WordWrap then
        begin
          vAuxPos.Row := cRow;
          if Assigned(fHighlighter) then
            ...
            vAuxPos.Column := 1;
          vFirstChar := DisplayToBufferPos(vAuxPos).Char;
          vAuxPos.Column := LastCol;
          vLastChar := DisplayToBufferPos(vAuxPos).Char;
        end;
        ...
        begin
          ...
          if (vFirstChar <> FirstCol) or (vLastChar <> LastCol) then
            sToken := Copy(sLineExpandedAtWideGlyphs, vFirstChar, vLastChar -
              vFirstChar)
          else
        ...
        begin
          ...
          begin
            ...
            begin
              ...
              if sToken = '' then
                raise
                  Exception.Create('The highlighter seems to be in an infinite loop');
            end;
            ...
            begin
              ...
              AddHighlightToken(sToken, nTokenPos - (vFirstChar - FirstCol),
                nTokenLen, attr.Foreground, attr.Background, attr.Style);
            end;
          ...
          begin
            ...
            begin
              ...
              begin
                ...
                AddHighlightToken(sToken, nTokenPos - (vFirstChar - FirstCol),
                  nTokenLen, colFG, colBG, Font.Style);
              end;
            end;
            // Draw LineBreak glyph.
            if (eoShowSpecialChars in fOptions) and (Length(sLine) < vLastChar)
              then
            begin
              AddHighlightToken(SynLineBreakGlyph,
                Length(sLine) - (vFirstChar - FirstCol),
                Length(SynLineBreakGlyph), colFG, colBG, Font.Style);
            end;
          end;
          PaintHighlightToken(True);
          //### Code Folding ###
          if (CodeFolding.Enabled) and (CodeFolding.CollapsedCodeHint)
            and (FoldRange <> nil) and (FoldRange.Collapsed)
            and (not FoldRange.ParentCollapsed) then
          begin
            Canvas.FillRect(rcToken);
            OldColor := Canvas.Brush.Color;
            Canvas.Brush.Color := clSilver;

            if Gutter.ShowLineNumbers and not WordWrap then
              FoldRange.HintMarkLeft := ((nTokenPos + nTokenLen) * CharWidth)
                + Gutter.LeftOffset + (GetAutoSizeDigitCount * CharWidth)
                + Gutter.RightOffset
            else if Gutter.ShowLineNumbers and (cRow=LastRowForLine(nLine)) then
            begin
              vAuxPos.Row := cRow;
              vAuxPos.Column := 1;
              FoldRange.HintMarkLeft := ((nTokenPos + nTokenLen-DisplayToBufferPos(vAuxPos).Char+2) * CharWidth)
                + Gutter.LeftOffset + (GetAutoSizeDigitCount * CharWidth)
                + Gutter.RightOffset
            end
            else
            ...
            begin
              ...
              Y := rcCollapseMark.Top + (((rcCollapseMark.Bottom -
                rcCollapseMark.Top) div 5) * 3);
              X := rcCollapseMark.Left + CharWidth - 2;
              Canvas.Rectangle(X, Y, X + 2, Y + 2);
              X := rcCollapseMark.Left + ((CharWidth * 3) div 2) - 1;
              Canvas.Rectangle(X, Y, X + 2, Y + 2);
              X := rcCollapseMark.Right - CharWidth;
              Canvas.Rectangle(X, Y, X + 2, Y + 2);
            end;
        ...
        if (CodeFolding.IndentGuides) and ((cRow = 1)
          or (RowToLine(cRow) <> RowToLine(cRow - 1))) then
        ...
        if (CodeFolding.IndentGuides) and (Lines.Count > 0) then
          if Length(IndentGuides[nLine - 1]) > 0 then
          begin
            Y := rcLine.Top;

            for i := 0 to High(IndentGuides[Pred(nLine)]) do
            begin
              X := IndentGuides[Pred(nLine)][i];

              if fHighlightedFoldRange <> fAllFoldRanges[i] then
              begin
                if LineHeight mod 2 = 0 then
                  Canvas.Draw(X, Y, fIndGuidBmp)
                else
                begin
                  if nLine mod 2 = 1 then
                    Canvas.Draw(X, Y, fIndGuidBmp)
                  else
                    Canvas.Draw(X, Y, fIndGuidOddBmp);
                end;
              end
              else
              begin
                if LineHeight mod 2 = 0 then
                  Canvas.Draw(X, Y, fHIndGuidBmp)
                else
                begin
                  if nLine mod 2 = 1 then
                    Canvas.Draw(X, Y, fHIndGuidBmp)
                  else
                    Canvas.Draw(X, Y, fHIndGuidOddBmp);
                end;
              end;
            end;
          end;

        if (CodeFolding.Enabled) and (CodeFolding.ShowCollapsedLine)
          and (FoldRange <> nil) and (FoldRange.Collapsed)
          and (not FoldRange.ParentCollapsed) then
        begin
          OldColor := Canvas.Pen.Color;
          ...
          Canvas.Pen.Style := CodeFolding.CollapsedLineStyle;
...
{$ENDIF}
  ...
  begin
    ...
    Canvas.Brush.Color := Color; //colEditorBG; KBFix
    Canvas.FillRect(rcToken);
...
{$ENDIF}
  ...
  begin
    if Highlighter <> nil then
      Highlighter.ResetRange;
    Canvas.Brush.Color := colEditorBG; // KBFix
    Canvas.FillRect(rcToken);
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          fBlockEnd := Value;
          if (fActiveSelectionMode <> smColumn) or (fBlockBegin.Char <>
            fBlockEnd.Char) then
...
begin
  if (not WordWrap) and (not CodeFolding.Enabled)  then
    Result := CaretY
  else
...
begin
  ...
  begin
    ...
    begin
      Dec(Result.Row);
      Result.Column := GetRowLength(Result.Row) + 1;
    end
    else
    begin
...
procedure TCustomSynEdit.SetCaretXYEx(CallEnsureCursorPos: Boolean; Value:
  TBufferCoord);
var
  nMaxX: Integer;
      ...
      smNormal:
        begin
          ...
          begin
            ...
            StringState :=TSynEditStringList(Lines).StringStates[BB.Line - 1];
            ...
            TSynEditStringList(Lines).StringStates[BB.Line - 1]:=StringState;
          end;
          UpdateMarks := True;
          InternalCaretXY := BB;
        end;
  ...
  procedure InsertText;
    ...
    begin
      ...
      begin
        ...
        StringState:=TSynEditStringList(flines).StringStates[CaretY];
        TSynEditStringList(Lines).InsertLines(CaretY, CountLines(P));
        i:=CaretY+CountLines(P)-1;
      end
      else
      begin
      ...
      begin
        ...
        ProperSetLine(CaretY - 1, Str);
        if (CaretY - 1)=i then
          if sLeftSide + sRightSide=Str then
            TSynEditStringList(flines).StringStates[i]:=StringState
          else
            TSynEditStringList(flines).StringStates[i]:=ssfChanged;
        Inc(Result);
      end;
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          FHScrollBar.LargeChange := CharsInWindow - Ord(eoScrollByOneLess in
            fOptions);
        ...
        begin
          ...
          FVScrollBar.LargeChange := LinesInWindow shr Ord(eoHalfPageScroll in
            fOptions);
...
procedure TCustomSynEdit.ScrollEvent(Sender: TObject; ScrollCode: TScrollCode;
        ...
        begin
          ...
          case FScrollHintFormat of
            ...
            s := Format(SYNS_ScrollInfoFmt, [RowToLine(TopLine),
              RowToLine(TopLine + Min(LinesInWindow, DisplayLineCount -
                TopLine))]);
          end;
...
begin
  ...
  fNeedRescanLineAndRow:=true;
  if CodeFolding.Enabled and (aCount>0) then
    begin
    for i := fAllFoldRanges.AllCount - 1 downto 0 do
      with fAllFoldRanges[i] do
        if (Collapsed) and (not ParentCollapsed) and
           (((aCount = 1) and (FromLine = aIndex)) or
           ((aCount > 1) and (FromLine >= aIndex) and (FromLine <=
           Pred(aIndex + aCount)))) then
          begin
  ...
  if (Lines.Count = 1) and (fAllFoldRanges.Count = 1) then
    fAllFoldRanges.Delete(0);
...
procedure TCustomSynEdit.InsertBlock(const BB, BE: TBufferCoord; ChangeStr:
  PWideChar;
  AddToUndoList: Boolean);
      ...
      repeat
        ...
        begin
          if FAutoComplete then
            ...
            FKeepGoing := True
              //### Code Folding ###
          else if Item.ChangeReason = crDeleteCollapsedFold then
            FKeepGoing := True
              //### End Code Folding ###
          else
          begin
...
begin
  ...
  if Assigned(Item) then
    try
      ExpandLine(Item.ChangeStartPos.Line);
      ActiveSelectionMode := Item.ChangeSelMode;
      IncPaintLock;
      Include(fOptions, eoScrollPastEol);
      case Item.ChangeReason of
          ...
          begin
            ...
            begin
              ...
              if (Length(TmpStr) < CaretX - 1)
                ...
                TmpStr := TmpStr + UnicodeStringOfChar(#32, CaretX - 1 -
                  Length(TmpStr));
      ...
      //undo StringStates
      if Assigned(Item.StringStates) then
        begin
        for i:=0 to fLines.Count-1 do
          begin
          TSynEditStringList(fLines).StringStates[i]:=
          TSynEditStringState(strtoint(Item.StringStates[i]));
          end;
        end;
    finally
      if ChangeScrollPastEol then
...
procedure TCustomSynEdit.ExpandAllParent(vFoldRange: TSynEditFoldRange);
var
  i, j: integer;
begin
  if vFoldRange.ParentCollapsed then
  begin
    for i := 0 to fAllFoldRanges.AllCount - 1 do
    begin
      if fAllFoldRanges.AllFoldRanges[i].SubFoldRanges <> nil then
      begin
        for j := 0 to fAllFoldRanges.AllFoldRanges[i].SubFoldRanges.Count - 1 do
        begin
          if fAllFoldRanges.AllFoldRanges[i].SubFoldRanges[j] = vFoldRange then
          begin
            ExpandAllParent(fAllFoldRanges.AllFoldRanges[i]);
            Expand(vFoldRange);
            Break;
          end;
        end;
...
begin
  ...
  begin
    ExpandLine(fBookMarks[BookMark].Line);
...
begin
  Result := (Msg = WM_SETTEXT) or (Msg = WM_GETTEXT) or (Msg =
    WM_GETTEXTLENGTH);
end;
...
{$ENDIF}
    ...
    begin
      ...
      begin
        ...
        vNewPos.Column := MinMax(vNewPos.Column, LeftChar, LeftChar +
          CharsInWindow - 1);
        vNewPos.Row := MinMax(vNewPos.Row, TopLine, TopLine + LinesInWindow -
          1);
...
begin
  ...
  begin
    ...
    case Command of
      ...
      begin
        //### Code Folding ###
        //expand range if edit her first line
        if CodeFolding.Enabled then
        begin
          //fix codefolding-2010-11-24_1853, codefolding-2010-11-24_1848
          ExpandLine(CaretY);
          if SelLength>0 then
          begin
            for i:=BlockBegin.Line to BlockEnd.Line do
            begin
              if IsLineInFoldRange(i) then
                 begin
                 fNeedToRescan := True;
                 Break;
                 end;
            ...
            //fix codefolding-2010-11-17_2122
            if (Command=ecDeleteLastChar) and (CaretY>1) and (CaretX=1) then
            begin
              ExpandLine(CaretY-1);
            end;
        end;

        // if something is going near keyword
        if IsKeywordAtCursorPos then
           fNeedToRescan := True;
      end;
    end;
    //### End Code Folding ###
...
procedure TCustomSynEdit.ExecuteCommand(Command: TSynEditorCommand; AChar:
  WideChar;
  Data: pointer);
...
var
  ...
  s: UnicodeString;
  i: Integer;
  tmpStringState: TSynEditStringState;
begin
  IncPaintLock;
  try
    case Command of
      ...
      ecGotoXY, ecSelGotoXY:
        ...
        begin
          MoveCaretAndSelection(CaretXY, TBufferCoord(Data^), Command =
            ecSelGotoXY);
          Update;
        end;
      // word selection
      ...
      ecDeleteLastChar:
        ...
        begin
          ...
          try
            ...
            begin
              Temp := LineText;
              TabBuffer := TSynEditStringList(Lines).ExpandedStrings[CaretY -
                1];
              Len := Length(Temp);
              Caret := CaretXY;
              vTabTrim := 0;
...
{$ENDIF}
                ...
                begin
                  ...
                  if (eoTrimTrailingSpaces in Options) and (Len <>
                    Length(TabBuffer)) then
      ...
      ecDeleteWord, ecDeleteEOL:
        ...
        begin
          ...
          begin
            WP := WordEnd;
            Temp := LineText;
            if (WP.Char < CaretX) or ((WP.Char = CaretX) and (WP.Line <
              fLines.Count)) then
            begin
              if WP.Char > Len then
              begin
...
{$ENDIF}
          ...
          begin
            ...
            Helper := SelText;
            SetSelTextPrimitive(UnicodeStringOfChar(' ', CaretX -
              BlockBegin.Char));
      ...
      ecDeleteLine:
        if not ReadOnly and (Lines.Count > 0) and not ((CaretY = Lines.Count)
          and (Length(Lines[CaretY - 1]) = 0)) then
        begin
          DoOnPaintTransient(ttBefore);
          if SelAvail then
            SetBlockBegin(CaretXY);
          ...
          begin
            Lines[CaretY - 1] := '';
            fUndoList.AddChange(crSilentDeleteAfterCursor, BufferCoord(1,
              CaretY),
        ...
        begin
          ...
          try
            ...
            begin
              ...
              begin
                if CaretX = 1 then
                   tmpStringState:=TSynEditStringList(Lines).StringStates[CaretY];
//                if CaretX > 1 then
                begin
                ...
                if CaretX = 1 then
                   TSynEditStringList(Lines).StringStates[CaretY-1]:=tmpStringState;
              end
              else
              begin
                SpaceCount2 := 0;
                ...
                if Command = ecLineBreak then
                begin
                  {Internal}CaretXY := BufferCoord(1, CaretY + 1);
      ...
      ecChar:
        ...
        begin
          ...
          begin
            ...
            begin
              if (Len > 0) then
                SpaceBuffer := UnicodeStringOfChar(#32, CaretX - Len -
                  Ord(fInserting))
              else
            ...
            try
              ...
              begin
                ...
                if (eoTrimTrailingSpaces in Options) and ((AChar = #9) or (AChar
                  = #32)) and (Length(TrimTrailingSpaces(LineText)) = 0) then
...
begin
  ...
  if CodeFolding.Enabled then
    if (fNeedToReScan)
      ...
      or (Command = ecUndo)
      or (Command = ecRedo)
      or (((Command = ecChar) or (Command = ecDeleteLastChar)) and
      (IsKeywordAtCursorPos)) then
      try
        ReScanForFoldRanges;
      finally
      end;

  if CodeFolding.IndentGuides then
    case Command of
  ...
  begin
    ...
    begin
      ...
      if IsIdentChar(Line[CX]) then
        CX := StrScanForCharInCategory(Line, CX, IsIdentCharNeg)
      else if IsWordBreakNoSpaceChar(Line[CX]) then
        CX := StrScanForCharInCategory(Line, CX, IsWordBreakNoSpaceCharNeg);

      if (CX > 0) and (CX <= LineLen) then
        if IsWhiteChar(Line[CX]) then
          CX := StrScanForCharInCategory(Line, CX, IsWhiteCharNeg);
...
begin
  ...
  begin
    ...
    // KBFix
    iLineLen := Length(Line);
    CX := Min(CX, iLineLen + 1);
    //    CX := Min(CX, Length(Line) + 1);
    ...
    begin
      // KBFix BEGIN
      if IsWhiteChar(Line[CX - 1]) then
        CX := StrRScanForCharInCategory(Line, CX - 1, IsWhiteCharNeg) + 1;

      if (CX > 1) and (CX - 1 <= iLineLen) then
        if IsWordBreakNoSpaceChar(Line[CX - 1]) then
          CX := StrRScanForCharInCategory(Line, CX - 1,
            IsWordBreakNoSpaceCharNeg) + 1
        else if IsIdentChar(Line[CX - 1]) then
          CX := StrRScanForCharInCategory(Line, CX - 1, IsIdentCharNeg) + 1;
      {
...
var
  ...
  begin
    ...
    begin
      ...
      Result := (First >= ptStart.Char) and (Last <= ptEnd.Char) or (ptEnd.Char
        - ptStart.Char < 1);
  end;
...
begin
  ...
  else if bChangeY and (DX = 1) and (ptO.Char > nLineLen) and (ptO.Line <
    Lines.Count) then
  begin
    // start of next line
    Inc(ptDst.Line);
...
procedure TCustomSynEdit.MoveCaretAndSelection(const ptBefore, ptAfter:
  TBufferCoord;
  SelectionCommand: Boolean);
begin
...
begin
  ...
  try
    ...
    else if (eoTrimTrailingSpaces in Options) and (StartOfBlock.Char >
      Length(LineText)) then
    begin
...
begin
  ...
  begin
    ...
    begin
      while (NewX > DestX) and ((NewX - 1 > LineLen) or (Line[NewX - 1] = #32))
        do
        dec(NewX);
    end;
  end;
...
procedure TCustomSynEdit.DoHomeKey(Selection: Boolean);
  ...
  begin
    ...
    begin
      vPos := DisplayXY;
      vPos.Column := Min(CharsInWindow, GetRowLength(vPos.Row) +
        1);
...
begin
  ...
  begin
    ...
    while (first_nonblank <= vMaxX) and
      CharInSet(s[first_nonblank], [#32, #9]) do
      inc(first_nonblank);
    dec(first_nonblank);
...
begin
  ...
  begin
    ...
    if WordWrap then
      ...
      vMinX := 0;
    while (vLastNonBlank > vMinX) and CharInSet(vText[vLastNonBlank], [#32, #9])
      do
      Dec(vLastNonBlank);
  ...
  begin
    ...
    if vEnhance then
      ...
      vNewCaret.Column := GetRowLength(vNewCaret.Row) + 1;
...
{$IFNDEF UNICODE}
  ...
  begin
    ...
    if Win32PlatformIsUnicode then
      SetWindowLongW(Handle, GWL_WNDPROC, Integer(GetWindowLongA(Handle,
        GWL_WNDPROC)));
  end;
{$ENDIF}
...
{$IFNDEF UNICODE}
  ...
  begin
    ...
    if Win32PlatformIsUnicode then
      SetWindowLongA(Handle, GWL_WNDPROC, Integer(GetWindowLongW(Handle,
        GWL_WNDPROC)));
  end;
{$ENDIF}
...
begin
  ...
  begin
    ...
    begin
      ...
      if fActiveSelectionMode = smColumn then
        Inc(Line, MinIntValue([BB.Char - 1, BE.Char - 1, Length(Lines[i -
            1])]));
...
var
  rcInval: TRect;
begin
  if (not HandleAllocated) or (Line < 1) or (Line > Lines.Count) or (not Visible)
    then
    Exit;
  ...
  begin
    ...
    rcInval := Rect(fGutterWidth, fTextHeight * (Line - TopLine), ClientWidth,
      0);
...
begin
  ...
  if (Sender = fUndoList) and not fUndoList.InsideRedo and
    (fUndoList.PeekItem <> nil) and (fUndoList.PeekItem.ChangeReason <>
    crGroupBreak) then
    fRedoList.Clear;
...
begin
  ...
  if (v_WordStart.Line = v_WordEnd.Line) and (v_WordStart.Char < v_WordEnd.Char)
    then
    Result := Copy(Lines[XY.Line - 1], v_WordStart.Char, v_WordEnd.Char -
      v_WordStart.Char)
  else
    Result := '';
end;

function TCustomSynEdit.BufferToDisplayRow(const line: Integer):Integer;
begin
  if WordWrap or CodeFolding.Enabled then
  begin
    if fNeedRescanLineAndRow then
      RescanLineAndRow;
    if (line > Lines.count) and (Line>1) then
      result:=0
    else
      result:=fRowByLine[line];
  end
  else
    Result:=line;
end;

function TCustomSynEdit.LastRowForLine(Line: Integer): Integer;
begin
  if WordWrap or CodeFolding.Enabled then
  begin
    if fNeedRescanLineAndRow then
    ...
    if line > Lines.count then
      ...
      begin
        result:=fRowByLine[line];
        while (result<fRowCount)and(fLineByRow[result+1]=Line) do
          Inc(result);
      end
  end
  else
...
function TCustomSynEdit.DisplayToBufferLine(const Row: Integer):Integer;
begin
  if WordWrap or CodeFolding.Enabled then
  begin
    if fNeedRescanLineAndRow then
      RescanLineAndRow;
    if (row > fRowCount) and (row>1) then
      result:=Row
    else
      Result:=fLineByRow[row];
  end
  else
    Result:=row;
end;
...
procedure TCustomSynEdit.RescanLineAndRow(Index: Integer);
var
  i,Row, lastRow, lastLine, line, LastWWRow,WWRow: Integer;
  buf:TBufferCoord;
  PrevLineCollapsed:Boolean;
  FoldRange:TSynEditFoldRange;
begin
  fNeedRescanLineAndRow:=False;
  if Index<=0 then
    Exit;
  if Length(fRowByLine)<fLines.Count+1 then
    SetLength(fRowByLine, fLines.Count+1);
  fRowByLine[0]:=0;
  fRowByLine[1]:=1;
  fLineByRow[0]:=0;
  fLineByRow[1]:=1;
  ...
  lastLine:=0;
  Row:=0;
  PrevLineCollapsed:=False;
  for line:=index to lines.count do
  begin
    if WordWrap then
      begin
      buf.Char:=1;
      buf.Line:=line;
      WWRow := fWordWrapPlugin.BufferToDisplayPos(buf).Row;
      end;
    if not PrevLineCollapsed and WordWrap then
      Inc(Row, WWRow-LastWWRow-1);
    FoldRange:=CollapsedFoldRangeByLine(line);
    if (FoldRange=nil) or (FoldRange.FromLine=line) then
    begin
      Inc(Row);
      fRowByLine[line]:=Row;
      if Row >= fLineByRowLength  then
        begin
        Inc(fLineByRowLength, 50);
        SetLength(fLineByRow, fLineByRowLength);
        end;
      for i:=lastRow+1 to Row-1 do
        fLineByRow[i]:=lastLine;
      fRowCount:=Row;
      fLineByRow[Row]:=line;
      lastRow:=Row;
      lastLine:=Line;
...
var
  ...
  FirstRowCoord:TDisplayCoord;
  LineBegin:TBufferCoord;
  FirstRow:Integer;
begin
  Canvas.Font := Font;
  ...
  Result.Row := BufferToDisplayRow(Result.Row);
  if WordWrap then
    begin
    LineBegin.Char:=1;
    LineBegin.Line:=p.Line;
    FirstRowCoord:=fWordWrapPlugin.BufferToDisplayPos(LineBegin);
    FirstRow:=BufferToDisplayRow(LineBegin.Line);
    Result := fWordWrapPlugin.BufferToDisplayPos(p);
    Result.Row:=Result.Row-FirstRowCoord.Row+FirstRow;
    end
end;
...
var
  ...
  i, L: Integer;
  x, CountOfAvgGlyphs: Integer;
  FirstRowCoord,ActualRowCoord:TDisplayCoord;
  Line:TBufferCoord;
  FirstRow:Integer;
begin
  Canvas.Font := Font;
  ...
  Result.Line := DisplayToBufferLine(p.Row);
  if WordWrap then
    begin
    Line.Char := 1;
    ...
    if Line.Line>0 then
    begin
      FirstRowCoord:=fWordWrapPlugin.BufferToDisplayPos(Line);
      FirstRow:=BufferToDisplayRow(Line.Line);
      ActualRowCoord.Column:=p.Column;
      ActualRowCoord.Row:=p.Row+FirstRowCoord.Row-FirstRow;
      if ActualRowCoord.Row>0 then
        Result := fWordWrapPlugin.DisplayToBufferPos(ActualRowCoord);
    end;
    end;
...
function TCustomSynEdit.GetRowLength(aRow: integer): integer;
var
  FirstRowCoord:TDisplayCoord;
  Line:TBufferCoord;
  FirstRow,ActualRow:Integer;
begin
  Result:=0;
  if (Lines.Count>0) then
  begin
    Result:=Length(Lines[DisplayToBufferLine(aRow)]);
    if WordWrap then
    begin
      Line.Char := 1;
      Line.Line := DisplayToBufferLine(aRow);
      if Line.Line>0 then
      begin
        FirstRowCoord:=fWordWrapPlugin.BufferToDisplayPos(Line);
        FirstRow:=BufferToDisplayRow(Line.Line);
        ActualRow:=aRow+FirstRowCoord.Row-FirstRow;
        if ActualRow>0 then
          Result := fWordWrapPlugin.GetRowLength(ActualRow);
      end
    end
  end
end;
...
procedure TCustomSynEdit.ProperSetLine(ALine: Integer; const ALineText:
  UnicodeString);
begin
  if eoTrimTrailingSpaces in Options then
  ...
  //Repaint for show line changes in gutter  
  if(fGutter.HighlightChanges) then InvalidateGutter; //Repaint;  
end;
...
function TCustomSynEdit.IsIdentCharNeg(AChar: WideChar): Boolean;
begin
  Result := not IsIdentChar(AChar);
end;
...
function TCustomSynEdit.IsWhiteCharNeg(AChar: WideChar): Boolean;
begin
...
function TCustomSynEdit.IsWordBreakCharNeg(AChar: WideChar): Boolean;
begin
...
function TCustomSynEdit.IsWordBreakNoSpaceChar(AChar: WideChar): Boolean;
begin
  case AChar of
    '.', ',', ';', ':', '"', '''', '´', '`', '°', '^', '!', '?', '&',
      {'$',}'@', '§', '%', '#', '~', '[', ']', '(', ')', '{', '}', '<', '>',
...
function TCustomSynEdit.IsWordBreakNoSpaceCharNeg(AChar: WideChar): Boolean;
begin
  Result := not IsWordBreakNoSpaceChar(AChar);
end;
...
begin
  ...
  if (Point.X < 0) or (Point.Y < 0) or (Point.X > Self.Width) or (Point.Y >
    Self.Height) then
  begin
    Result := False;
    Exit;
...
begin
  ...
  if (UpperCase(SelText) = UpperCase(FindDialog.FindText)) and not (frReplace in
...
begin
  ...
  begin
    Result := BufferToDisplayRow(aLine);
  end;
end;
...
begin
  ...
  begin
    Result := DisplayToBufferLine(aRow);
  end;
end;
...
begin
  ...
  fCaretAtEOL := WordWrap and (aPos.Row <= DisplayLineCount) and
    (aPos.Column > GetRowLength(aPos.Row)) and
    (DisplayY <> aPos.Row);
  DecPaintLock;
  UpdateLastCaretX;
end;
...
begin
  ...
  Lines.EndUpdate;
  InvalidateGutter;
  InvalidateLines(AFoldRange.FromLine, Lines.Count);
end;
...
begin
  ...
  for i := fAllFoldRanges.AllCount - 1 downto 0 do
    with fAllFoldRanges[i] do
      if (RealLevel = ALevel) and (not Collapsed) and (not ParentCollapsed)
        and (Collapsable) then
...
function TCustomSynEdit.GetLineIndent(Strings: TUnicodeStrings;
...
var
  i,j:Integer;
  FirstLinesFromCollapsedRanges: array of Integer;
  FoldRange: TSynEditFoldRange;
begin
  fNeedToRescan := False; // Set rescan flag to false

  SetLength(FirstLinesFromCollapsedRanges, fAllFoldRanges.AllCount);
  // Save FirstLines from collapsed ranges and delete all folds
  j:=0;
  for i := fAllFoldRanges.AllCount - 1 downto 0 do
    with fAllFoldRanges[i] do
    begin
      if Collapsed then
      begin
        FirstLinesFromCollapsedRanges[j]:=FromLine;
        Inc(j);
      end;
    end;
  ...
  //Restore (if possible) collapsed state for ranges
  for i := 0 to j-1 do
  begin
    FoldRange:=CollapsableFoldRangeForLine(FirstLinesFromCollapsedRanges[i]);
    if Assigned(FoldRange) then
      Collapse(FoldRange)
  end;

  InvalidateGutter;
  Finalize(FirstLinesFromCollapsedRanges);
end;

procedure TCustomSynEdit.ScanForFoldRanges(
  var TopFoldRanges: TSynEditAllFoldRanges; Strings: TUnicodeStrings);
var
  Line, FoldCount: Integer;
  Parents: TList;
  Ptr, TmpPtr, KeyWordPtr: PChar;
  ...
  var
    ...
    begin
      Result := (not (FirstChar^ in TSynValidStringChars))
        and (not (LastChar^ in TSynValidStringChars));
    end;
    ...
    begin
      ...
      repeat
        // Win
        if ((Ptr^ = #13) and ((Ptr + 1)^ = #10)) or ((Ptr^ = #10) and ((Ptr + 1)^
          = #13)) then
          repeat
            Inc(Ptr, 2);
            Inc(Line);
            Result := True;
          until not (((Ptr^ = #13) and ((Ptr + 1)^ = #10)) or ((Ptr^ = #10) and
            ((Ptr + 1)^ = #13)));
    ...
    begin
          ...
          rtChar:
            begin
              if (Ptr^ = FoldRegions[i].Open^)
                and ((not FoldRegions[i].WholeWords) or (IsWholeWord(Ptr - 1, Ptr
                + 1))) then
              begin // Found an open character
                if Parents.Count > 0 then
                  FoldRanges := TSynEditFoldRange(Parents[Parents.Count -
                    1]).SubFoldRanges
                else
          ...
          rtKeyWord:
            begin
              ...
              begin
                ...
                while (Ptr^ <> #0) and (KeyWordPtr^ <> #0) and (Ptr^ =
                  KeyWordPtr^) do
                begin
                  Inc(Ptr);
                  Inc(KeyWordPtr);
              ...
              begin
                ...
                if (KeyWordPtr^ = #0) and ((not FoldRegions[i].WholeWords) or
      ...
      begin
        ...
        begin
          ...
          if (KeyWordPtr^ = #0)
            and ((not ParentRegion.WholeWords) or (IsWholeWord(TmpPtr - 1, Ptr)))
              then
            ParentClosed := True
          else
            Ptr := TmpPtr;
...
function TCustomSynEdit.TreeEndForRow(Row: Integer): Boolean;
var
  i, Line: Integer;
begin
  Result := False;
  ...
  for i := 0 to fAllFoldRanges.AllCount - 1 do
    with fAllFoldRanges[i] do
      if (not ParentCollapsed) and (not Collapsed) and (ToLine = Line)
        and (Collapsable) then
      begin
        Result := True;
        Break;
      end;
  if (Result = True) and WordWrap then
    begin
    if Line=Lines.Count then
      Result := fRowCount=Row
    else
      Result := Row=LastRowForLine(line);
    end;

end;

function TCustomSynEdit.TreeLineForRow(Row: Integer): Boolean;
var
  i,Line: Integer;
  isToLine, isFromLine:Boolean;
begin
  Result := False;
  isToLine:=False;
  isFromLine:=False;
  ...
  for i := 0 to fAllFoldRanges.AllCount - 1 do
    with fAllFoldRanges[i] do
      if (not ParentCollapsed) and (not Collapsed) and (Collapsable)
        and (FromLine <= Line) and (ToLine >= Line) then
      begin
        Result := True;
        if ToLine = Line then isToLine:=True;
  ...
  if isFromLine and WordWrap and (Row<>LineToRow(line)) then
      Result := True;
  if isToLine then
    if WordWrap then
      Result := Row<>LastRowForLine(line)
...
procedure TCustomSynEdit.Expand(FoldRange: TSynEditFoldRange);
begin
  Lines.BeginUpdate;
  ...
  Lines.EndUpdate;
  InvalidateGutter;
  InvalidateLines(FoldRange.FromLine, Lines.Count);
end;

procedure TCustomSynEdit.ExpandLine(Line: Integer);
var
  FoldRange: TSynEditFoldRange;
...
procedure TCustomSynEdit.ExpandLevel(ALevel: Integer; NeedInvalidate:
  Boolean);
var
  i: Integer;
begin
  ...
  for i := 0 to fAllFoldRanges.AllCount - 1 do
    with fAllFoldRanges[i] do
      ...
      begin
        ...
        begin
          if not CodeFolding.CaseSensitive then
            Line := UpperCase(Copy(Lines[FromLine - 1], CaretX, MaxInt))
          else
            Line := Copy(Lines[FromLine - 1], CaretX, MaxInt);
        ...
        begin
          ...
          begin
            if not CodeFolding.CaseSensitive then
              Line := UpperCase(Copy(Lines[ToLine - 1], CaretX, MaxInt))
            else
              Line := Copy(Lines[ToLine - 1], CaretX, MaxInt);
  ...
  begin
    ...
    begin
      ...
      repeat
        ...
        if (P > 0) and ((CaretX >= P)
          and (CaretX <= P + Integer(StrLen(Keyword)))) then
...
function TCustomSynEdit.FoldRangeForLineTo(
  Line: Integer): TSynEditFoldRange;
var
  i: Integer;
begin
  Result := nil;
...
function TCustomSynEdit.IsLineInFoldRange(Line: Integer): Boolean;
var
  i: Integer;
begin
  Result := false;
  if Line<=lines.count then
    for i := 0 to fAllFoldRanges.AllCount - 1 do
      with fAllFoldRanges[i] do
        if (FromLine <= Line) and (ToLine >= Line) then
        begin
          Result := True;
          exit;
        end
...
function TCustomSynEdit.CollapsedFoldRangeByLine(Line: Integer): TSynEditFoldRange;
var
  i,j: Integer;
begin
  Result := nil;
  if Line<=lines.count then
    for j := 9 downto 0 do
      for i := 0 to fAllFoldRanges.AllCount - 1 do
        with fAllFoldRanges[i] do
          if (RealLevel = j) and (not ParentCollapsed) and (FromLine <= Line)
            and (ToLine >= Line) and (Collapsed) then
          begin
            Result := fAllFoldRanges[i];
            exit;
          end
...
procedure TCustomSynEdit.ExpandCurrent;
var
  Nearest, i: Integer;
begin
  Nearest := -1;
  ...
  for i := 0 to fAllFoldRanges.AllCount - 1 do
    with fAllFoldRanges[i] do
      if (not (Collapsed or ParentCollapsed)) and (FromLine <= CaretY + 1)
        and (ToLine > CaretY + 1) then
      begin
        if Nearest = -1 then
          Nearest := i
        else if FromLine > fAllFoldRanges[Nearest].FromLine then
  ...
  if Nearest <> -1 then
  begin
    Expand(fAllFoldRanges[Nearest]);
    Invalidate;
  end;
end;

function TCustomSynEdit.GetCollapseMarkRect(
  FoldRange: TSynEditFoldRange; Row: Integer): TRect;
begin
...
procedure TSynEditMarkList.GetMarksForLine(line: Integer; var marks:
  TSynEditMarks);
...
procedure TCustomSynEdit.LoadFromFile(FileName:string);
begin
...
procedure TCustomSynEdit.LoadFromStream(Stream: TStream);
begin
...
procedure TCustomSynEdit.SaveToFile(FileName:string);
var i:Integer;
begin
  Lines.BeginUpdate;
  ...
  for i:=0 to fLines.Count-1 do
    begin
    if TSynEditStringList(fLines).StringStates[i]=ssfChanged then
      TSynEditStringList(fLines).StringStates[i]:=ssfSaved;
    end;
  Lines.EndUpdate;
  InvalidateGutter;
end;

procedure TCustomSynEdit.SaveToStream(Stream: TStream);
var i:Integer;
begin
  Lines.BeginUpdate;
...
{$IFNDEF SYN_CLX}
{$IFNDEF UNICODE}
  if Win32PlatformIsUnicode and not (csDesigning in Application.ComponentState)
    then
  begin
